...
export function StationsPage() {
  ...
  const flagMutation = useMutation({
    mutationFn: async ({
      ...
      if (readback[flag] !== apiTarget)
        throw new Error(
          'Drova не подтвердил новое состояние. Показано фактическое значение.',
        );
      return readback;
    },
    ...
    onSuccess: (readback) => {
      queryClient.setQueryData<Station[]>(
        ['stations', mode, account?.uuid],
        (current = []) =>
          current.map((item) =>
            item.uuid === readback.uuid ? readback : item,
          ),
      );
    },
    onError: (error) => {
      setRowError(
        error instanceof Error ? error.message : 'Не удалось изменить станцию.',
      );
    ...
    <div className="mx-auto max-w-[1500px] px-4 py-7 md:px-7 md:py-9">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1.5 max-w-xl text-sm text-muted-foreground">
            Управляйте публикацией, доступом и обновлениями без перехода между
            страницами.
          </p>
        </div>
      ...
      {!account && !stationsQuery.isPending ? (
        ...
        <div className="mt-7 rounded-2xl border border-dashed p-12 text-center">
          <ServerOff className="mx-auto size-6 text-muted-foreground" />
          <h2 className="mt-4 font-semibold">Станций пока нет</h2>
...
function DescriptionDialog({
  ...
  const save = form.handleSubmit(async ({ description: nextDescription }) => {
    ...
    try {
      ...
      const readback = await api.getStation(station.uuid, account.uuid);
      if (readback.description !== nextDescription)
        throw new Error('Drova не подтвердил сохранённое описание.');
      queryClient.setQueryData<Station[]>(
        ['stations', mode, account.uuid],
        (current = []) =>
          current.map((item) =>
      ...
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-3xl">
        ...
        <Tabs defaultValue="edit" className="mt-1">
          ...
          <TabsContent value="edit" className="mt-3 space-y-2">
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                Скрипты, формы, стили и небезопасные ссылки удаляются из
                preview.
              </p>
              <span className="text-xs tabular-nums text-muted-foreground">
          ...
          <TabsContent value="preview" className="mt-3">
            <div
              className="description-preview min-h-64 rounded-xl border bg-background p-5"
              dangerouslySetInnerHTML={{
                __html: preview || '<p>Описание пустое.</p>',
              }}
            />
          </TabsContent>
        </Tabs>
        {station && (
          <Button
            ...
            render={
              <a
                href={`https://drova.io/stations/${encodeURIComponent(station.uuid)}`}
                target="_blank"
                rel="noreferrer"
              />
            }
...
function EmptyConnection({ onOpenSettings }: { onOpenSettings(): void }) {
  return (
    <div className="mt-7 rounded-2xl border border-dashed p-12 text-center">
      <Wifi className="mx-auto size-6 text-primary" />
      <h2 className="mt-4 font-semibold">Подключите merchant token</h2>
      <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
        Либо переключитесь на демо-режим, чтобы посмотреть интерфейс без
        запросов в Drova.
      </p>
